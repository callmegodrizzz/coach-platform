 ...
 *
 *  ⚠️  СЕМИНАРСКАЯ НЕДЕЛЯ ЧАСТИЧНО ПО ДОПУЩЕНИЮ. Понедельник добавлен с твоих
 *      слов: немецкий в 8:00 и ТО1 в 12:20 (фото семинарской недели было
 *      обрезано слева, колонка понедельника в кадр не попала).
 *      Осталось уточнить: аудитория немецкого в понедельник (сейчас '?')
 *      и есть ли на этой неделе немецкий ещё и в среду с пятницей.
 */
...
export const LESSONS: Lesson[] = [
  // ── Понедельник (только семинарская неделя) ────────────────────────
  // На лекционной неделе понедельник пустой — видно на фото за 31.08.
  { subjectId: 'de', weekday: 1, slot: 1, room: '?', kind: 'practice', weeks: 'seminars' },
