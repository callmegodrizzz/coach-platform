 ...
 *
 *  Понедельник заполнен со слов владельца расписания (на фото эта колонка
 *  была обрезана): немецкий в 8:00 каждую неделю, ТО1 в 12:20 через неделю.
 *  Проверено по датам: 07.09 и 21.09 — с ТО1, 14.09 и 28.09 — без него.
 *
 *  ⚠️  Осталось уточнить аудиторию немецкого в понедельник — сейчас стоит '?'.
 *      Остальные пары семинарской недели повторяют сетку лекционной.
 */
...
export const LESSONS: Lesson[] = [
  // ── Понедельник ────────────────────────────────────────────────────
  // Немецкий идёт каждую неделю, ТО1 — только на семинарской.
  // На фото за 31.08 понедельник пустой, потому что это была первая неделя
  // семестра и занятия начались со вторника, 1 сентября.
  { subjectId: 'de', weekday: 1, slot: 1, room: '?', kind: 'practice', weeks: 'both' },
  { subjectId: 'to1', weekday: 1, slot: 3, room: '26', kind: 'seminar', weeks: 'seminars' },
  ...
  // ТО1 на семинарской неделе перенесён в понедельник — на фото за неделю
  // это единственный блок ТО1.с, дублировать его в пятницу было ошибкой.
];
